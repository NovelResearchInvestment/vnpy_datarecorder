...
class DataRecorderEngine(BaseEngine):
    ...
    def load_setting(self):
        ...
        if "data_recorder_setting.data" in self.main_engine.global_setting:
            self.data_setting = self.main_engine.global_setting["data_recorder_setting.data"]
        else:
            self.data_setting = load_json(self.data_recorder_setting)

        if len(self.data_setting) == 0:
            raise ValueError(f"data_recorder_setting.data are not auto-generated by main scripts; plus, {self.data_recorder_setting} is empty.")
